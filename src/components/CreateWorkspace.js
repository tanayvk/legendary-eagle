...
function CreateForm(props)
{
   ...
   let classes = "py-3 px-4 border border-gray-400 focus:outline-none rounded-md focus:ring-1 ring-cyan-500 w-full transition-all ";
   ...
   return <div class="mb-4 mt-4 ">
        <form class = "space-y-2 ">
            <input type="text" placeholder="Workspace Handle" className={classes} value={props.workspaceName} />
            <input type="text" placeholder="Shh.. Top secret!" className={classes} value={props.workspacePassword} />
        </form>
    </div>
}
...
class CreateWorkspace extends React.Component
{
    ...
    renderWorkspaceButton(){
        return( <button className={"w-full text-white p-3 rounded-full bg-gradient-to-r from-green-400 to-blue-500 hover:from-pink-500 hover:to-yellow-500 font-semibold text-lg focus:border focus:border-2 "} onClick={this.handleCreate}>
            { this.state.pressedCount > 0 ? "Boring! Generate again." : "Create new Workspace" }
        </button>
        );
    }
    renderCreateLoading(){
